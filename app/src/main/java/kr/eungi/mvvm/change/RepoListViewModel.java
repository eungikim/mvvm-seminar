...
public class RepoListViewModel extends AndroidViewModel {
    ...
    SingleLiveEvent<Boolean> mProgressBarEvent = new SingleLiveEvent<>();
    SingleLiveEvent<String> mSnackbarEvent = new SingleLiveEvent<>();
    MutableLiveData<List<GitHubService.RepositoryItem>> mRepositoryesData = new MutableLiveData<>();
    ...
    void loadRepositories(String language) {
        // 로딩 중이므로 진행바를 표시한다
        mProgressBarEvent.setValue(true);
        ...
        observable.subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread()).subscribe(
                (repositories) -> {
                    ...
                    mProgressBarEvent.setValue(false);
                    // 가져온 아이템을 표시하고자 RecyclerView에 아이템을 설정하고 갱신한다
                    mRepositoryesData.postValue(repositories.items);
                },
                (e) -> {
                    // 통신 실패 시에 호출된다
                    // 여기서는 스낵바를 표시한다(아래에 표시되는 바)
                    mProgressBarEvent.setValue(false);
                    mSnackbarEvent.setValue("읽어올 수 없습니다.");
                }
        );
    }

    public SingleLiveEvent<Boolean> getmProgressBarEvent() {
        return mProgressBarEvent;
    }

    public SingleLiveEvent<String> getmSnackbarEvent() {
        return mSnackbarEvent;
    }

    public MutableLiveData<List<GitHubService.RepositoryItem>> getmRepositoryesData() {
        return mRepositoryesData;
    }
}
